...
export const createCustomItem = async (item) => {
  const { name, make, model, exterior, wheels, interior, price } = item

  const result = await pool.query(
    `INSERT INTO custom_items (name, make, model, exterior, wheels, interior, price)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING *`,
    [name, make, model, exterior, wheels, interior, price]
  )
...
export const updateCustomItem = async (id, item) => {
  ...
  const result = await pool.query(
    `UPDATE custom_items
     SET name = $1, make = $2, model = $3, exterior = $4, wheels = $5, interior = $6, price = $7
     WHERE id = $8
     RETURNING *`,
    [name, make, model, exterior, wheels, interior, price, id]
  )
